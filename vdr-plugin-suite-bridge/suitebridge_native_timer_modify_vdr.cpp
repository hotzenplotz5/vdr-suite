...
bool validDay(const std::string &value)
{
  ...
  for (std::size_t i=0;i<value.size();++i)
    if (i!=4 && i!=7 && std::isdigit(static_cast<unsigned char>(value[i]))==0)
      return false;
  return true;
}
bool validWeekdays(const std::string &value)
{
  ...
  for (std::size_t i=0;i<7;++i)
    if (value[i]!='-' && value[i]!=Names[i]) return false;
  return true;
}
std::string timerDay(const SuiteBridgeNativeTimerModifyRequest &r, bool &valid)
{
  valid=validWeekdays(r.weekdays);
  if (!valid) return {};
  const bool repeating=r.weekdays.find_first_not_of('-')!=std::string::npos;
  if (repeating) {
    valid=validDay(r.day);
    return valid ? r.weekdays+(r.day.empty()?std::string():"@"+r.day) : std::string();
  }
  valid=validDay(r.day) && !r.day.empty();
  return valid ? r.day : std::string();
}
std::string timerFile(const SuiteBridgeNativeTimerModifyRequest &r, bool &valid)
{
  valid=!r.title.empty() && r.title.find('~')==std::string::npos &&
      r.title.find('|')==std::string::npos && r.directory.find('|')==std::string::npos;
  if (!valid) return {};
  std::string value=r.directory.empty()?r.title:r.directory+"~"+r.title;
...
{
  return std::string("ntmod:vdr:") + reason + ":" + commandId;
}
...
SuiteBridgeNativeTimerModifyVdrMutationCallback::ModifyTimer(
    const SuiteBridgeNativeTimerModifyRequest &request)
{
  int timerId=0;
  if (!parseLocalTimerId(request.backendNativeTimerId,timerId))
    return result(SuiteBridgeNativeTimerModifyMutationDisposition::RejectedWithoutEffect,
  ...
  try {
    ...
    const std::string fingerprint=canonical.empty()?std::string():sha256Token(canonical);
    if (fingerprint.empty() || fingerprint!=request.expectedNativeTimerFingerprint) {
      stateKey.Remove(false);
      return result(SuiteBridgeNativeTimerModifyMutationDisposition::RejectedWithoutEffect,
                    evidence("fingerprint-mismatch",request.commandId));
    }
    if (timer->Recording() || timer->Pending()) {
      stateKey.Remove(false);
      return result(SuiteBridgeNativeTimerModifyMutationDisposition::RejectedWithoutEffect,
                    evidence(timer->Recording()?"recording":"pending",request.commandId));
    }

    cTimer candidate=*timer;
    if (request.kind==SuiteBridgeNativeTimerModifyKind::Toggle) {
      if (request.enabled) candidate.SetFlags(tfActive);
      else candidate.ClrFlags(tfActive);
    } else {
      bool valid=false;
      const std::string selectedDay=timerDay(request,valid);
      const std::string selectedFile=valid?timerFile(request,valid):std::string();
      if (!valid || request.startTime.size()!=4 || request.endTime.size()!=4 ||
          !digits(request.startTime) || !digits(request.endTime)) {
        stateKey.Remove(false);
        return result(SuiteBridgeNativeTimerModifyMutationDisposition::RejectedWithoutEffect,
                      evidence("invalid-specification",request.commandId));
      }
      const unsigned flags=(request.enabled?tfActive:tfNone) |
          (request.vps?tfVps:tfNone);
      std::ostringstream definition;
      definition << flags << ':' << request.channelId << ':' << selectedDay << ':'
                 << request.startTime << ':' << request.endTime << ':'
                 << request.priority << ':' << request.lifetime << ':'
                 << selectedFile << ':' << (timer->Aux()?timer->Aux():"");
      if (!candidate.Parse(definition.str().c_str())) {
        stateKey.Remove(false);
        return result(SuiteBridgeNativeTimerModifyMutationDisposition::RejectedWithoutEffect,
                      evidence("parse-rejected",request.commandId));
      }
    }
    const bool recording=timer->Recording();
    timers->SetExplicitModify();
    *timer=candidate;
    timer->SetRecording(recording);
    timers->SetModified();
    stateKey.Remove();
    return result(SuiteBridgeNativeTimerModifyMutationDisposition::AppliedUnverified,
                  evidence("modified-unverified",request.commandId));
  } catch (...) {
    return result(SuiteBridgeNativeTimerModifyMutationDisposition::OutcomeUnknown,
