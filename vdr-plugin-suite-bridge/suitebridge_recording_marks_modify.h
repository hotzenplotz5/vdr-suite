#ifndef VDR_SUITE_BRIDGE_RECORDING_MARKS_MODIFY_H
#define VDR_SUITE_BRIDGE_RECORDING_MARKS_MODIFY_H

#include "suitebridge_command_result.h"

#include <cstddef>
#include <cstdint>
#include <mutex>
#include <string>
#include <unordered_map>

enum class SuiteBridgeRecordingMarksModifyKind {
  Add,
  Delete,
  Move,
  Clear,
};

struct SuiteBridgeRecordingMarksModifyRequest final {
  SuiteBridgeRecordingMarksModifyKind kind = SuiteBridgeRecordingMarksModifyKind::Add;
  std::string commandId;
  std::string requestFingerprint;
  std::string operationId;
  std::string operationRevision;
  std::string recordingKey;
  std::string expectedMarksRevision;
  int sourceFrame = -1;
  int targetFrame = -1;
  std::string jobId;
  std::string attemptId;
  std::uint64_t claimEpoch = 0;
  std::string backendId;
  std::string agentId;
  std::string agentInstanceId;
  std::uint64_t backendGeneration = 0;
  std::uint64_t controlPlaneClaimedAt = 0;
  std::string authorityDomain;
  std::string providerId;
  std::string providerKind;
  std::uint64_t ownershipGeneration = 0;
  std::string providerInstanceEpoch;
  std::uint64_t providerGeneration = 0;
  std::uint64_t capabilityRevision = 0;
  std::string requiredCapability;
  std::uint64_t localStartingPersistedAt = 0;
};

enum class SuiteBridgeRecordingMarksModifyMutationDisposition {
  AppliedUnverified,
  RejectedWithoutEffect,
  OutcomeUnknown,
};

struct SuiteBridgeRecordingMarksModifyMutationResult final {
  SuiteBridgeRecordingMarksModifyMutationDisposition disposition =
      SuiteBridgeRecordingMarksModifyMutationDisposition::OutcomeUnknown;
  std::string evidenceReference;
};

class ISuiteBridgeRecordingMarksModifyMutationCallback {
public:
  virtual ~ISuiteBridgeRecordingMarksModifyMutationCallback() = default;
  virtual SuiteBridgeRecordingMarksModifyMutationResult ModifyMarks(
      const SuiteBridgeRecordingMarksModifyRequest &request) = 0;
};

class SuiteBridgeRecordingMarksModifyService final {
public:
  explicit SuiteBridgeRecordingMarksModifyService(
      std::string pluginInstanceEpoch,
      ISuiteBridgeRecordingMarksModifyMutationCallback *mutationCallback = nullptr,
      std::size_t maximumReplayEntries = 4096);

  SuiteBridgeCommandResult Handle(const char *command, const char *option);
  bool ExecutionConfigured() const noexcept;

private:
  struct ReplayEntry final {
    std::string commandId;
    std::string requestFingerprint;
    std::string canonicalRequest;
    bool terminal = false;
    SuiteBridgeRecordingMarksModifyMutationResult result;
  };

  SuiteBridgeCommandResult capability(const char *option) const;
  SuiteBridgeCommandResult execute(const char *option);
  SuiteBridgeRecordingMarksModifyMutationResult executeReserved(
      const SuiteBridgeRecordingMarksModifyRequest &request,
      const std::string &canonicalRequest,
      bool &inProgress,
      bool &conflict,
      bool &ledgerFull);

  std::string pluginInstanceEpoch_;
  ISuiteBridgeRecordingMarksModifyMutationCallback *mutationCallback_ = nullptr;
  std::size_t maximumReplayEntries_ = 4096;
  std::mutex replayMutex_;
  std::unordered_map<std::string, ReplayEntry> replayByOperationId_;
  std::unordered_map<std::string, std::string> operationByCommandId_;
};

#endif
