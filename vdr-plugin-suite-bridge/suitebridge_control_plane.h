...
public:
 ...
 struct Metrics { std::array<std::uint64_t,4> admittedByOperation{};std::array<std::uint64_t,4> executedByOperation{};std::uint64_t rejected=0,overloaded=0,deadlineExpiredBeforeExecution=0,protocolMismatch=0,peerRejected=0,queueHighWaterMark=0,queueWaitNanoseconds=0,executionNanoseconds=0;std::size_t queueDepth=0;bool running=false;};
