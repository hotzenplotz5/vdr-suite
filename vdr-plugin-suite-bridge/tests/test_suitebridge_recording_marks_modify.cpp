...
int main()
{
  ...
  auto capability = service.Handle("NMARKS", "CAP 1 modify");
  require(
      capability.handled && capability.replyCode == 900 &&
          capability.payload.find(
              "vdr.recording.marks.modify 1 recording-marks-modify enabled") !=
              std::string::npos,
      "enabled capability missing");

  SuiteBridgeRecordingMarksModifyService disabled("pie_1");
  capability = disabled.Handle("NMARKS", "CAP 1 modify");
  require(
  ...
  reply = disabled.Handle(
      "NMARKS",
      execute("clear", "-", "-", "cmd_disabled", "op_disabled").c_str());
