...
.PHONY: test-phase68-osd-controller-lease
test-phase68-osd-controller-lease:
	$(BUILD_CXX) $(CXXFLAGS) -pthread \
		core/daemon/src/LegacyOsdSessionService.cpp \
		core/daemon/src/OsdViewerBindingService.cpp \
		core/daemon/src/OsdControllerLeaseService.cpp \
		api/rest/src/LegacyOsdApiRuntime.cpp \
		core/daemon/tests/test_osd_controller_lease_service.cpp \
		$(LDFLAGS) \
		-o $(BUILD_DIR)/test_osd_controller_lease_service
	$(BUILD_DIR)/test_osd_controller_lease_service
	$(MAKE) test-security-osd-session
	python3 tools/check_phase68_osd_view_session_authorization.py
	python3 tools/check_phase68_osd_viewer_bindings.py
...
test-phase68-osd-native-input:
	$(BUILD_CXX) $(CXXFLAGS) 		core/vdr/tests/test_legacy_osd_input_domain.cpp 		-o $(BUILD_DIR)/test_legacy_osd_input_domain
	$(BUILD_DIR)/test_legacy_osd_input_domain
	$(BUILD_CXX) $(CXXFLAGS) -pthread 		$(sort $(SQLITE_SRC) $(AGENT_CONTROL_PLANE_SRC)) 		core/security/src/AccountabilityEventRepository.cpp 		core/security/src/CredentialVerifierRepository.cpp 		core/security/src/SecurityIdentityRepository.cpp 		core/security/src/SecurityIdentityProvisioningRepository.cpp 		core/vdr/src/VdrConfig.cpp 		core/vdr/src/BackendRegistry.cpp 		core/vdr/src/BackendRegistryService.cpp 		core/daemon/src/LegacyOsdSessionService.cpp 		core/daemon/src/OsdViewerBindingService.cpp 		core/daemon/src/OsdControllerLeaseService.cpp 		core/daemon/src/LegacyOsdInputService.cpp 		core/daemon/tests/test_legacy_osd_input_service.cpp 		$(LDFLAGS) 		-o $(BUILD_DIR)/test_legacy_osd_input_service
