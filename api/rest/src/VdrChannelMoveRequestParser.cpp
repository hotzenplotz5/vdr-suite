...
VdrChannelMoveRequest VdrChannelMoveRequestParser::parse(
    const std::string& body) const
{
    const auto values =
        parseFlatObject(body);

    VdrChannelMoveRequest request;
    ...
    request.channelId =
        getValue(values, "channelId");

    request.sourceNumber =
        parseFirstInt(
            values,
            {"sourceNumber", "source", "from", "number"},
            0);

    request.targetNumber =
        parseFirstInt(
            values,
            {"targetNumber", "target", "to"},
            0);

    request.dryRun =
        parseBool(values, "dryRun", false);
