...
agent_test = read("core/agent/tests/test_suite_bridge_svdrp_native_timer_delete_transport.cpp")
...
plugin_test = read("vdr-plugin-suite-bridge/tests/test_suitebridge_native_timer_delete.cpp")
...
require(agent_sources, "AGENT_NATIVE_TIMER_DELETE_TRANSPORT_SRC :=", "dedicated typed Timer-delete transport source set")
...
transport_block = agent_sources.split("AGENT_NATIVE_TIMER_DELETE_TRANSPORT_SRC :=", 1)[1].split("\n\nAGENT_OBSERVATION_SRC", 1)[0]
require(transport_block, source_path, "dedicated Timer-delete transport source ownership")
generic_block = agent_sources.split("AGENT_SVDRP_TRANSPORT_SRC :=", 1)[1].split("\n\nAGENT_NATIVE_TIMER_DELETE_TRANSPORT_SRC", 1)[0]
forbid(generic_block, source_path, "Timer-delete source in generic SVDRP source set")

forbid(svdrp_header, "public IBackendAgentNativeTimerDeleteTransport", "Timer-delete interface on generic SuiteBridge SVDRP transport")
require(svdrp_header, "SuiteBridgeCommandReply discoverNativeTimerDeleteContract();", "typed raw capability hook")
require(svdrp_header, "SuiteBridgeCommandReply executeNativeTimerDeleteContract(", "typed raw execute hook")
require(adapter_header, "class SuiteBridgeNativeTimerDeleteTransport final :", "dedicated Timer-delete adapter")
require(adapter_header, "public IBackendAgentNativeTimerDeleteTransport", "typed Timer-delete interface implementation")
...
for needle, label in (
    ...
    ("command.operationId", "operation wire identity"),
    ...
    ("EnabledState", "enabled successor capability state"),
    ("native_timer_delete_suitebridge_provider_discovered_enabled", "enabled provider discovery"),
    ("BackendAgentNativeTimerDeleteTransportDisposition::rejectedWithoutEffect", "definitive rejection mapping"),
    ("BackendAgentNativeTimerDeleteTransportDisposition::acceptedUnverified", "accepted-unverified successor mapping"),
    ("BackendAgentNativeTimerDeleteTransportDisposition::outcomeUnknown", "ambiguous transport mapping"),
    ("AcceptedUnverifiedReplyCode", "typed accepted reply code"),
    ("OutcomeUnknownReplyCode", "typed unknown reply code"),
):
    require(transport_source, needle, label)
for token in ('"DELT"', "cTimers", "Timers->", "/timers", "RESTfulAPI", "restfulapi"):
    forbid(transport_source, token, "direct VDR Timer mutation coupling in Agent transport")
...
require(plugin_main, "mutations=enabled execution=enabled", "truthful successor mutation log")
...
require(plugin_makefile, "suitebridge_native_timer_delete.o", "plugin Timer-delete service object build")
...
for needle, label in (
    ...
    ("BackendAgentNativeTimerDeleteTransportDisposition::acceptedUnverified", "accepted-unverified Agent regression"),
    ("ntdel:vdr:deleted-unverified:cmd_1", "native apply evidence regression"),
    ("suitebridge:ntdel:reply-fence-mismatch", "post-dispatch fence mismatch regression"),
    ("expectedNativeTimerFingerprint.clear()", "missing fingerprint rejection regression"),
    ("expectedNativeTimerFingerprint.back() = 'g'", "malformed digest rejection regression"),
):
...
for needle, label in (
    ...
    ("reentrantReply.replyCode == 558", "plugin in-progress replay regression"),
):
...
require(doc, "No real VDR Timer deletion", "historical Slice-34 no-mutation documentation")
...
print("Phase 64 SuiteBridge Timer-delete transport guard passed (real-mutation successor aware)")
