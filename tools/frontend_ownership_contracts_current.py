#!/usr/bin/env python3
"""Current frontend ownership contracts for the modular VDR-Suite web runtime."""
...
def combined_install_source() -> str:
    return "\n".join(
        (
            read(ROOT / "mk/install.mk"),
            read(ROOT / "mk/recordings2.mk"),
        )
    )
...
def check_index_contract(index_html: str) -> None:
    ...
    core.require(
        ".recording-list-item" not in index_html
        and ".recording-detail::before" not in index_html,
...
def run_baseline_client_api_contract() -> None:
    """Run the still-valid baseline while replacing two obsolete alias assertions."""
...
def check_frontend_asset_table_contract(
    server_source: str,
    install_source: str,
) -> None:
    core.require(
        "const FrontendAsset FRONTEND_ASSETS[]" in server_source,
        "TestHttpServer must own the table-driven frontend asset registry",
    )

    required_assets = (
        ("/frontend/platform/deferred-runtime-loader.js", "platform/deferred-runtime-loader.js"),
        ("/frontend/channel-browser.js", "channel-browser.js"),
        ("/frontend/modules/channels.js", "modules/channels.js"),
        ("/frontend/channel-day-program.js", "channel-day-program.js"),
        ("/frontend/channel-day-program-compat.js", "channel-day-program-compat.js"),
        ("/frontend/epg-searchtimer-actions.js", "epg-searchtimer-actions.js"),
        ("/frontend/recordings2-shared.js", "recordings2-shared.js"),
        ("/frontend/recordings2-folder-artwork.js", "recordings2-folder-artwork.js"),
        ("/frontend/recordings2-actions.js", "recordings2-actions.js"),
        ("/frontend/recordings2-browser-view.js", "recordings2-browser-view.js"),
        ("/frontend/recordings2-person-search-view.js", "recordings2-person-search-view.js"),
        ("/frontend/recordings2-metadata-view.js", "recordings2-metadata-view.js"),
        ("/frontend/recordings2-metadata-detail.js", "recordings2-metadata-detail.js"),
        ("/frontend/recordings2.js", "recordings2.js"),
    )

    for request_path, relative_path in required_assets:
        table_entry = f'{{"{request_path}", "{relative_path}"'
        core.require(
            table_entry in server_source,
            f"frontend asset registry is missing mapping {request_path} -> {relative_path}",
        )
        core.require(
            f"web/frontend/{relative_path}" in install_source
            or f"$(DATADIR)/web/frontend/{relative_path}" in install_source,
            f"runtime installation is missing frontend asset {relative_path}",
        )
    ...
    for retired_path in (
        ...
        core.require(
            ...
            f"HTTP server still exposes retired frontend asset: {retired_path}",
        )
...
def check_install_contract(install_mk: str) -> None:
    ...
    for token in required_absence_checks:
        core.require(
            token in install_mk,
            f"test-install-staging missing retirement assertion: {token}",
        )
...
def main() -> int:
    try:
        ...
        install_source = combined_install_source()
        ...
        check_frontend_asset_table_contract(server_source, install_source)
        core.check_frontend_static_serving_contract(
            index_html,
            server_source,
            install_source,
        )
