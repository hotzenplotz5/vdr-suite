...
old_detail = '''function renderEpgEventDetail(event, channel, sourceElement) {
...
new_detail = '''function renderEpgEventDetail(event, channel, sourceElement) {
  selectedEpgDetail = { event, channel };
...
if "function renderMobileEpgInlineDetail" not in app:
    app = replace_once(app, old_detail, new_detail, "mobile detail block")

old_guard = '''  const nowSeconds = Math.floor(Date.now() / 1000);
  const bounds = epgTimelineBounds(nowSeconds);
  const limit = EPG_VISIBLE_CHANNEL_LIMIT;
'''

new_guard = '''  const nowSeconds = Math.floor(Date.now() / 1000);
...
if old_guard in app:
    app = replace_once(app, old_guard, new_guard, "mobile view guard")

old_hint = '''  header.appendChild(cacheStatus);

  const modeRow = document.createElement('div');
'''

new_hint = '''  header.appendChild(cacheStatus);
...
if old_hint in app and "epg-mobile-mode-note" not in app:
    app = replace_once(app, old_hint, new_hint, "mobile hint")

if "timeView.classList.add('epg-desktop-only');" not in app:
    app = replace_once(
        app,
        "  timeView.className = 'epg-view-button ' + (epgProgramView === 'horizontal' ? 'active' : '');\n",
        "  timeView.className = 'epg-view-button ' + (epgProgramView === 'horizontal' ? 'active' : '');\n  timeView.classList.add('epg-desktop-only');\n",
        "desktop-only horizontal button",
    )

if "verticalTimeView.classList.add('epg-desktop-only');" not in app:
    app = replace_once(
        app,
        "  verticalTimeView.className = 'epg-view-button ' + (epgProgramView === 'vertical' ? 'active' : '');\n",
        "  verticalTimeView.className = 'epg-view-button ' + (epgProgramView === 'vertical' ? 'active' : '');\n  verticalTimeView.classList.add('epg-desktop-only');\n",
        "desktop-only vertical button",
    )
...
print("Phase 58.91 mobile EPG inline expansion applied")
