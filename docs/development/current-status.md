...
Current active runtime slice: **69.C - Revision/precondition/idempotency exposure**. PR #321 established the shared ETag / precondition foundation, PR #322 accepted one actor-scoped `MutationOperationReadService`, PR #323 accepted authenticated durable-operation reads, PR #324 accepted the backend-scoped read-only `TimerAssignmentReadService`, PR #325 accepted exactly one DaemonRuntime `TimerAssignmentRepository` plus lifecycle-safe PublicApiRuntime lookup, PR #326 accepted authenticated `GET /api/v1/timer-assignments/{timerAssignmentId}?backend={backendId}` with pre-lookup `timers.view`, a deliberately minimal representation, strong opaque `ETag` and `If-None-Match -> 304`, PR #327 accepted the dormant native Timer CREATE preparation owner, PR #328 accepted the dormant durable reservation/dispatch/activation owners, and PR #329 accepted the durable internal NativeTimerSpecification on new TimerAssignments. The active bounded step is [Native Timer CREATE Fulfillment Runtime Composition](phase-69c-native-timer-create-fulfillment-runtime.md): compose the existing Phase-64 `TimerAssignmentFulfillmentService` and one `NativeTimerBindingRepository` over the production daemon database so the later public admission can use the accepted revision-fenced `selected -> provisioning` owner. Both fulfillment methods remain dormant, PublicApiRuntime and SecurityHttpGate remain read-only for TimerAssignment, and native execution remains closed. Phase 69.A route/resource inventory and Phase 69.B request/error foundations are accepted; see [Phase 69.B Closeout](phase-69b-closeout.md) and [Phase 69 Public API Kickoff and Runtime Progress](phase-69-public-api-kickoff.md).
