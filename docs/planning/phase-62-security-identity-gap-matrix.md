...
Implemented branch slices: Slice 1; real-runtime-accepted lifecycle, managed Basic, browser-session issuance/logout, ordinary-route browser authentication, persisted actor grants, Remote CSRF, Webfrontend session integration and Slice 2E.1 runtime hardening; Slice 2F fixed-role repository implementation pending installed acceptance
...
| Roles | Fixed `role.admin` and `role.read-only` assignments use existing backend-scoped actor-grant persistence; exact-scope `read-only` wins for the current protected mutation and `admin` expands only to the explicit `remote.control` catalogue | `AuthorizationService` and `SecurityPermissionGrantRepository` | Installed-runtime acceptance; later generic definitions, assignments and administration | Broad or wildcard role semantics could over-grant | Slice 2F runtime acceptance | focused authorization and grant-repository tests |
...
| Central authorization | Remote migrated; browser precedence, persisted grants, role constraints and unavailable-store denial are enforced; all other browser-authenticated business POSTs remain fail-closed | `SecurityHttpGate` and `AuthorizationService` | Complete route permission mapping, safe-POST classification and business CSRF enforcement | Most legacy routes still rely on compatibility bypass | One business route family per slice | general/dedicated gate, authorization and real-yaVDR evidence |
...
2. **Slice 2 — active**  
   Real-runtime accepted through Slice 2E.1: persistence, lifecycle resolution, managed Basic verifier, browser issuance/logout, ordinary-route browser authentication, backend-scoped grants, Remote CSRF, Webfrontend session integration and runtime hardening. Slice 2F adds the fixed exact-scope `admin`/`read-only` model at repository level; installed-runtime acceptance remains pending. Still required: further business-route migration, completion accountability, cleanup/recovery and native/service credential lifecycle.
...
Phase 62 remains open. The next strict work after Slice 2F repository and installed acceptance is one bounded Webfrontend mutation route family—not Android or later-phase runtime.
