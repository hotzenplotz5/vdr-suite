# VDR-Suite Roadmap

## Navigation

- [README](../../README.md)
- [Documentation Index](../index.md)
- [Project Overview](../project-overview.md)
- [Planning Index](index.md)

---

## Current Position

```text
Completed implementation state
Phase 46.9 - Content Rating Resolution Model

Next implementation step
Phase 46.10 - Content Rating JSON Contract
```

---

## Purpose

This roadmap describes the forward direction of VDR-Suite.

Current project status belongs to:

- [Current Project Status](../development/current-status.md)
- [Project Status Dashboard](../project-status-dashboard.md)

Completed implementation history belongs to:

- [Completed Phases](../development/completed-phases.md)

---

## Current Direction

The current work continues the content classification foundation from ADR-0028.

The completed genre block defines canonical IDs, source evidence, localized labels and JSON serialization.

The next block extends the same classification model to content rating data.

---

## Roadmap Rule

This file describes direction, not detailed implementation history.

Completed implementation detail belongs in [Completed Phases](../development/completed-phases.md).

---

## Back

- [Back to README](../../README.md)
- [Back to Documentation Index](../index.md)
- [Back to Project Overview](../project-overview.md)
