...
(async function () {
  ...
  const dramaRecordings = Array.from({length: 20}, (_, index) => ({
    ...
    backendId: 'default',
    backendNativeId: 'native-drama-' + String(index + 1),
  ...
  assert.strictEqual(await api.loadRandomGenre(
    ...
    'default',
    0,
    {id: 'drama', label: 'Drama', count: dramaRecordings.length}
  ), true);
  ...
  assert.deepStrictEqual(recordingIds(randomSection), dramaRecordings.slice(0, 12).map((recording) => recording.recordingId));
  ...
  ).includes('fetchAllSeriesRecordings('));
