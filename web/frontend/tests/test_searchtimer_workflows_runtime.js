...
assert.strictEqual(api.normalizeDirectory('/Movies/Action/'), 'Movies~Action');
assert.strictEqual(api.recordingFolderPathToDirectory('/Series'), 'Series');
...
assert.deepStrictEqual(
  Array.from(api.directoryOptions(
    {folders: [{name: 'Movies', path: 'Movies'}, {name: 'Series', path: '/Series'}]},
    {searchTimers: [{recordingOptions: {directory: 'News'}}, {directory: 'Movies~Action'}]},
    'Archive~Old'
  )),
...
const channels = [
  ...
  api.normalizeChannel({id: 'S19.2E-1-1101-28106', number: 4, name: 'Radio Test', group: ''}, 3)
];
assert.deepStrictEqual(Array.from(api.channelGroups(channels, true)), ['Öffentlich-rechtlich', 'Privat', '__ungrouped__']);
assert.deepStrictEqual(Array.from(api.channelGroups(channels, false)), ['Öffentlich-rechtlich', 'Privat']);
assert.deepStrictEqual(
...
assert.strictEqual(api.inferChannelMode(true, 'Privat', '', ''), 2);
assert.strictEqual(api.inferChannelMode(true, '', 'A', 'A'), 1);
...
function baseElements() {
  return {
    ...
    channelFilterGroup: field('select-one', 'Öffentlich-rechtlich'),
...
const groupPayload = api.buildCreatePayload({elements: baseElements()}, timer);
...
assert.strictEqual(groupPayload.channels, 'Öffentlich-rechtlich');
assert.strictEqual(groupPayload.channelMin, '');
assert.strictEqual(groupPayload.channelMax, '');
assert.deepStrictEqual(Array.from(api.validateCreatePayload(groupPayload)), []);

const singleElements = baseElements();
singleElements.channelFilterMode.value = '1';
singleElements.channelFilterGroup.value = '';
singleElements.channelId.value = 'S19.2E-1-1011-11110';
singleElements.manualDirectory.value = '/Series/Tatort/';
const singlePayload = api.buildCreatePayload({elements: singleElements}, timer);
assert.strictEqual(singlePayload.directory, 'Series~Tatort');
assert.strictEqual(singlePayload.useChannel, 1);
assert.strictEqual(singlePayload.channels, '');
assert.strictEqual(singlePayload.channelMin, 'S19.2E-1-1011-11110');
assert.strictEqual(singlePayload.channelMax, 'S19.2E-1-1011-11110');
assert.deepStrictEqual(Array.from(api.validateCreatePayload(singlePayload)), []);

const allElements = baseElements();
allElements.channelFilterMode.value = '0';
allElements.channelFilterGroup.value = '';
const allPayload = api.buildCreatePayload({elements: allElements}, timer);
...
const intervalPayload = api.buildCreatePayload({elements: intervalElements}, timer);
...
assert.ok(source.includes("field('Aufnahmeverzeichnis auswählen', directorySelect, true)"));
assert.ok(source.includes("field('Kanalfilter', modeSelect, true)"));
