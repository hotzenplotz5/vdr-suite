(function(global) {
  ...
  function listFromResponse(data, key) {
    ...
    if (data && Array.isArray(data.items) && key === 'recordings') {
      return data.items;
    }
    return [];
  }
  ...
  function displayPath(path) {
    ...
    return normalized === ''
      ? 'Aufnahmen'
      : normalized.split('/').map(decodeVdrText).join(' / ');
  }
  ...
  function recordingMeta(recording) {
    ...
    if (start !== '') {
      ...
      if (Number.isFinite(epoch) && epoch > 1000000000) {
        values.push(new Date(epoch * 1000).toLocaleString('de-DE'));
      } else {
        values.push(String(start));
      }
    }
    if (Number.isFinite(duration) && duration > 0) {
      values.push(String(Math.round(duration / 60)) + ' min');
    }
    if (Number.isFinite(sizeMb) && sizeMb > 0) {
      values.push(sizeMb >= 1024 ? (sizeMb / 1024).toFixed(1) + ' GB' : Math.round(sizeMb) + ' MB');
    }
    return values.join(' · ');
  }
  ...
  function renderRecordingDetail(recording, folderData, index) {
    ...
    const subtitle = firstValue(recording, ['shortText', 'subtitle', 'description'], '');
    if (subtitle !== '') {
      detail.appendChild(textElement('p', decodeVdrText(subtitle)));
    }

    const meta = recordingMeta(recording);
    if (meta !== '') {
      detail.appendChild(textElement('p', meta, 'list-meta'));
    }

    const description = firstValue(recording, ['description', 'longText', 'summary'], '');
    if (description !== '' && description !== subtitle) {
      detail.appendChild(textElement('p', decodeVdrText(description)));
    }
  ...
  function makeInteractiveRow(title, meta, onOpen, extraClass) {
    ...
    if (meta) {
      row.appendChild(textElement('div', meta, 'list-meta'));
    }
  ...
  function renderFolder(data) {
    ...
    header.appendChild(textElement(
      'p',
      String(folders.length) + ' Ordner · ' + String(recordings.length) + ' Aufnahmen'
    ));
    ...
    if (path !== '') {
      ...
      container.appendChild(makeInteractiveRow(
        'Zurück',
        displayPath(backPath),
        function() { loadFolder(backPath, 0); }
      ));
    }
  ...
  if (global.VdrSuitePlatform && typeof global.VdrSuitePlatform.registerModule === 'function') {
    if (!global.VdrSuitePlatform.hasModule('recordings')) {
