...
@media(max-width:720px){.channels2-toolbar{align-items:stretch;flex-direction:column}.channels2-tools{display:grid;grid-template-columns:1fr}.channels2-filters{grid-template-columns:1fr}.channels2-filter-buttons{flex-wrap:nowrap;overflow-x:auto;padding-bottom:.15rem}.channels2-filter-button{flex:0 0 auto}.channels2-group-channels{padding:.3rem}.channels2-inline-program{margin-inline:0;padding:.55rem}.channels2-program-head{align-items:stretch;flex-direction:column}.channels2-date{display:grid;grid-template-columns:auto 1fr auto}.channels2-date input{grid-column:1/-1;width:100%}.channels2-event{grid-template-columns:4.8rem minmax(0,1fr)}.channels2-event .channels2-badge{display:none}.channels2-detail.has-artwork{grid-template-columns:1fr}.channels2-artwork{min-height:10rem}.channels2-actions{display:grid;grid-template-columns:1fr 1fr}.channels2-actions button{width:100%;font-size:.9rem}}
`;
  ...
  function renderEventDetail(event, channel) {
    ...
    art.className = 'channels2-artwork';
    ...
    copy.className = 'channels2-detail-copy';
  ...
  function renderInlineProgram(channel) {
    ...
    prev.type = today.type = next.type = 'button';
    const input = document.createElement('input');
    input.type = 'date';
    input.value = dateValue(state.day);
    prev.onclick = () => { state.day = moveDay(state.day, -1); loadEvents(); };
    today.onclick = () => { state.day = dayOnly(new Date()); loadEvents(); };
    next.onclick = () => { state.day = moveDay(state.day, 1); loadEvents(); };
    input.onchange = () => { state.day = dayOnly(input.value); loadEvents(); };
    controls.append(prev, today, next, input);
    head.append(title, controls);
    section.appendChild(head);
    const day = addText(document.createElement('p'), dayOnly(state.day).toLocaleDateString('de-DE', {weekday: 'long', day: '2-digit', month: 'long', year: 'numeric'}));
