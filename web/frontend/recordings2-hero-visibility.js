...
(function (global) {
  ...
  function detailFieldValue(root, label) {
    ...
    for (let index = 0; index < fields.length; index += 1) {
      ...
      if (String(fieldLabel.textContent || '').trim() !== label) continue;
      return String(fieldValue.textContent || '').trim();
    }
    return '';
  }

  function normalizeFallbackFact(label, value) {
    let normalized = String(value || '').trim();
    if (!normalized || normalized === '–' || normalized === 'Dauer unbekannt') return '';

    if (label === 'Bewertung') {
      const rating = Number(normalized.replace(',', '.').split('/')[0].trim());
      return Number.isFinite(rating) && rating > 0
        ? '★ ' + rating.toFixed(1) + ' / 10'
        : '';
    }

    if (label === 'Veröffentlichung') {
      const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(normalized);
      if (match) normalized = match[3] + '.' + match[2] + '.' + match[1];
    }
  ...
  function syncHeroFactsFromDetail(root) {
    if (!root || typeof root.querySelector !== 'function' || !global.document) return;
    const facts = root.querySelector('.recordings2-hero-facts');
    if (!facts) return;

    const present = elements(facts, '.recordings2-hero-fact-label').map(function (entry) {
      return String(entry.textContent || '').trim();
    });

    ['Bewertung', 'Veröffentlichung', 'Genre', 'Dauer', 'Größe'].forEach(function (label) {
      if (present.indexOf(label) !== -1) return;
      const value = normalizeFallbackFact(label, detailFieldValue(root, label));
      if (!value) return;

      const fact = global.document.createElement('span');
      fact.className = 'recordings2-hero-fact' + (label === 'Bewertung' ? ' rating' : '');
      const factLabel = global.document.createElement('span');
      factLabel.className = 'recordings2-hero-fact-label';
      factLabel.textContent = label;
      const factValue = global.document.createElement('span');
      factValue.className = 'recordings2-hero-fact-value';
...
.recordings2-hero-page[data-recordings2-hero-mode="detail"] .recordings2-detail-copy>.recordings2-hero-actions{display:contents}
.recordings2-hero-page[data-recordings2-hero-mode="detail"] .recordings2-detail-copy>.recordings2-hero-actions>button.primary{order:4}
...
.recordings2-hero-page[data-recordings2-hero-mode="metadata"]>.recordings2-metadata-panel{position:relative;z-index:2;width:calc(100% - clamp(2rem,6vw,6rem));max-width:80rem;margin:5.5rem auto 0!important}
.recordings2-hero-page[data-recordings2-hero-mode="metadata"]>.recordings2-metadata-assignment,.recordings2-hero-page[data-recordings2-hero-mode="metadata"]>[data-recordings2-metadata-assignment-error]{position:relative;z-index:2;width:calc(100% - clamp(2rem,6vw,6rem));max-width:80rem;margin:1rem auto 0!important}
.recordings2-hero-page[data-recordings2-hero-mode="metadata"]>.recordings2-metadata-tabs{position:relative;z-index:2;width:calc(100% - clamp(2rem,6vw,6rem));max-width:80rem;margin:1rem auto 3rem!important}
...
.recordings2-hero-page[data-recordings2-hero-mode="playback"]>.recordings2-volume-owner-shell,.recordings2-hero-page[data-recordings2-hero-mode="marks"]>.recordings2-volume-owner-shell{position:relative;z-index:2;display:grid;grid-template-columns:minmax(0,1fr);justify-items:stretch;width:calc(100% - var(--recordings2-technical-inline-space));max-width:var(--recordings2-technical-max-width);margin:5.5rem auto 1rem}
...
.recordings2-hero-page .recordings2-volume-owner-shell,.recordings2-hero-page .recordings2-track-owner-shell,.recordings2-hero-page .recordings2-recording-fallback-shell,.recordings2-hero-page .recordings2-recording-fallback-transport,.recordings2-hero-page .recordings2-playback,.recordings2-hero-page .recordings2-playback-controls,.recordings2-hero-page .recordings2-volume-controls,.recordings2-hero-page .recordings2-track-controls{width:100%;min-width:0}
.recordings2-hero-page .recordings2-playback-transport{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(6.5rem,1fr))!important;width:100%;min-width:0}
.recordings2-hero-page .recordings2-playback-transport>button{width:100%;min-width:0}
.recordings2-hero-page .recordings2-playback-position,.recordings2-hero-page .recordings2-playback-direct-seek,.recordings2-hero-page .recordings2-playback-controls input[type="range"]{width:100%;min-width:0}
.recordings2-hero-page[data-recordings2-hero-mode="playback"]>.recordings2-metadata-panel.recordings2-hero-recording-panel,.recordings2-hero-page[data-recordings2-hero-mode="marks"]>.recordings2-metadata-panel.recordings2-hero-recording-panel{position:relative;z-index:2;width:calc(100% - var(--recordings2-technical-inline-space));max-width:var(--recordings2-technical-max-width);margin:0 auto!important;padding:0;border:0;background:transparent}
