(function (global) {
  ...
  const number = (value, fallback) => Number.isFinite(Number(value)) ? Number(value) : Number(fallback || 0);
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .genres-header p{margin:.3rem 0 0;color:#94a3b8}.genres-scope{display:flex;gap:.5rem;padding:.3rem;border:1px solid #334155;border-radius:.9rem;background:#0f172a}
      ...
      .genres-summary{display:flex;gap:.65rem;flex-wrap:wrap}.genres-summary span{padding:.45rem .7rem;border:1px solid #334155;border-radius:999px;background:#0f172a;color:#cbd5e1}
      ...
      .genre-card:hover,.genre-card:focus-visible{transform:translateY(-2px);border-color:#38bdf8;outline:none}.genre-card img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.72}
      .genre-card:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.94))}.genre-card-copy{position:absolute;z-index:1;left:1rem;right:1rem;bottom:1rem;display:grid;gap:.22rem}
      .genre-card-title{font-size:1.25rem;font-weight:900}.genre-card-count{color:#bae6fd;font-weight:800}.genre-card-state{color:#cbd5e1;font-size:.8rem}
      .genres-status{padding:1rem;border:1px solid #334155;border-radius:1rem;background:#0f172a;color:#cbd5e1}.genres-status.error{border-color:#ef4444;color:#fecaca}
      .genres-recordings{display:grid;grid-template-columns:repeat(auto-fill,minmax(17rem,1fr));gap:1rem}.genres-epg-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(19rem,1fr));gap:1rem}
      .genres-epg-card{overflow:hidden;border:1px solid #334155;border-radius:1rem;background:#0f172a;color:#e2e8f0;text-align:left;padding:0;cursor:pointer}.genres-epg-card img{display:block;width:100%;aspect-ratio:16/9;object-fit:cover;background:#020617}
      .genres-epg-copy{display:grid;gap:.35rem;padding:.9rem}.genres-epg-title{font-size:1.03rem;font-weight:850;color:#f8fafc}.genres-epg-meta{font-size:.82rem;color:#93c5fd}.genres-epg-description{font-size:.86rem;color:#cbd5e1;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
      .genres-owned-detail{display:grid;gap:.8rem;grid-column:1/-1}@media(min-width:72rem){.genres-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.genre-card{min-height:17rem}}
  ...
  function genreArtworkUrl(id) {
    const assets = {action:'action',documentary:'doku',drama:'drama',music:'musik',musical:'musical',mystery:'mystery','science-fiction':'scifi',series:'serien',thriller:'thriller',western:'western'};
    return assets[id]
      ? '/channel-logos/vdr-suite-brand/recording-genre-' + assets[id] + '.svg'
  ...
  function formatTime(epoch) {
    ...
    if (value <= 0) return '–';
    return new Intl.DateTimeFormat('de-DE', {weekday:'short',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(value * 1000));
  }

  function genres() {
    return state.overview && Array.isArray(state.overview.genres) ? state.overview.genres : [];
  }

  function renderHeader(root) {
    const header = node('header', 'genres-header');
    const copy = node('div');
    copy.append(node('h3', '', 'Genres'), node('p', '', 'Persistente Metadatenansicht für Aufnahmen und EPG.'));
    const scope = node('div', 'genres-scope');
    [['recordings','Aufnahmen'],['epg','EPG']].forEach(entry => {
      const item = button(entry[1], state.scope === entry[0] ? 'active' : '', () => {
  ...
  function renderOverview(root) {
    ...
    summary.append(
      ...
      node('span', '', genres().length + ' Genres'),
      node('span', '', 'Backend ' + state.backendId)
    );
    root.appendChild(summary);
    const grid = node('section', 'genres-grid');
    genres().forEach(genre => {
      const card = button('', 'genre-card', () => selectGenre(genre));
      ...
      copy.append(
        node('span', 'genre-card-title', genre.label || genre.labelDe || genre.id),
        node('span', 'genre-card-count', number(genre.count, 0) + ' Treffer')
      );
      const conditions = [];
      if (number(genre.staleCount, 0)) conditions.push(genre.staleCount + ' veraltet');
      if (number(genre.conflictCount, 0)) conditions.push(genre.conflictCount + ' Konflikte');
      if (!genre.known) conditions.push('nicht kanonisch');
      if (conditions.length) copy.appendChild(node('span', 'genre-card-state', conditions.join(' · ')));
  ...
  function closeLabel() {
    return '← Zurück zu ' + text(state.selectedGenre && state.selectedGenre.label, 'Genre');
  }
  ...
  function openRecording(recording) {
    ...
    owner.openRecording(recording, {backendId:state.backendId,backLabel:closeLabel(),onClose:() => {state.active=true;render();}});
  }
  ...
  function openEpg(event) {
    ...
    owner.open(event, channelFor(event), {backLabel:closeLabel(),onClose:() => {state.active=true;render();}});
  }

  function renderResults(root) {
    const header = node('div', 'genres-result-header');
    header.append(
      button('← Alle Genres', 'genres-back', () => {state.selectedGenre=null;state.items=[];render();}),
      node('h4', '', text(state.selectedGenre && (state.selectedGenre.label || state.selectedGenre.id)) + ' · ' + state.total + ' Treffer')
    );
    root.appendChild(header);
    ...
    if (state.scope === 'recordings') {
      ...
      state.items.forEach(recording => {
        if (cardOwner && typeof cardOwner.createRecordingCard === 'function') {
          list.appendChild(cardOwner.createRecordingCard(recording, openRecording));
        }
      });
  ...
  function render() {
    ...
    renderHeader(root);
    if (state.loading) root.appendChild(node('section', 'genres-status', 'Genres werden aus dem persistenten Index geladen …'));
    else if (state.error) root.appendChild(node('section', 'genres-status error', state.error.message || state.error));
  ...
  function requestItems(offset) {
    const client = api();
    const options = {backendId:state.backendId,genreId:state.selectedGenre.id,limit:PAGE_SIZE,offset:offset,cache:'no-store',credentials:'same-origin'};
    if (state.scope === 'recordings') return client.fetchClientGenreRecordings(options);
    const now = Math.floor(Date.now() / 1000);
    return client.fetchClientGenreEpg(Object.assign({}, options, {from:now,until:now+172800}));
  }
  ...
  if (tab) {
    tab.addEventListener('click', () => {
      document.querySelectorAll('.module-tab').forEach(entry => entry.classList.toggle('active', entry === tab));
  ...
  if (!platform.hasModule || !platform.hasModule('genres')) platform.registerModule('genres', moduleApi);
