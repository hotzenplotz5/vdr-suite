...
(function(global) {
  ...
  function clientApi() {
    if (searchTimerBrowserContext.clientApi) {
      return searchTimerBrowserContext.clientApi;
    }
  ...
  function normalizeSearchTimer(searchTimer, index) {
    ...
    return {
      ...
      active: state === 'active' || boolValue(
        firstValue(source, ['active', 'enabled', 'isActive', 'use_as_searchtimer'], false),
        false
      ),
      ...
      marginStartMinutes: numberValue(
        firstValue(schedule, ['marginStartMinutes'], firstValue(source, ['margin_start', 'marginStart'], 0)),
        0
      ),
      marginStopMinutes: numberValue(
        firstValue(schedule, ['marginStopMinutes'], firstValue(source, ['margin_stop', 'marginStop'], 0)),
        0
      ),
      useVps: boolValue(
        firstValue(schedule, ['useVps'], firstValue(source, ['use_vps', 'vps'], false)),
        false
      ),
      useChannel: boolValue(
        firstValue(filters, ['useChannel'], firstValue(source, ['use_channel', 'useChannel'], false)),
        false
      ),
      channels: String(firstValue(channels, ['channels'], firstValue(source, ['channels'], ''))),
      channelMin: numberValue(
        firstValue(channels, ['channelMin'], firstValue(source, ['channel_min', 'channelMin'], 0)),
        0
      ),
      channelMax: numberValue(
        firstValue(channels, ['channelMax'], firstValue(source, ['channel_max', 'channelMax'], 0)),
        0
      ),
      useDuration: boolValue(
        firstValue(filters, ['useDuration'], firstValue(source, ['use_duration', 'useDuration'], false)),
        false
      ),
      durationMinMinutes: numberValue(
        firstValue(filters, ['durationMinMinutes'], firstValue(source, ['duration_min', 'durationMin'], 0)),
        0
      ),
      durationMaxMinutes: numberValue(
        firstValue(filters, ['durationMaxMinutes'], firstValue(source, ['duration_max', 'durationMax'], 0)),
        0
      ),
      useDayOfWeek: boolValue(
        firstValue(filters, ['useDayOfWeek'], firstValue(source, ['use_dayofweek', 'useDayOfWeek'], false)),
        false
      ),
      compareTitle: boolValue(
        firstValue(comparison, ['compareTitle'], firstValue(source, ['compare_title', 'use_title'], true)),
        true
      ),
      compareSubtitle: boolValue(
        firstValue(comparison, ['compareSubtitle'], firstValue(source, ['compare_subtitle', 'use_subtitle'], false)),
        false
      ),
      compareSummary: boolValue(
        firstValue(comparison, ['compareSummary'], firstValue(source, ['compare_summary', 'use_description'], false)),
        false
      ),
      compareCategories: boolValue(
        firstValue(comparison, ['compareCategories'], firstValue(source, ['compare_categories'], false)),
        false
      ),
      compareTime: boolValue(
        firstValue(comparison, ['compareTime'], firstValue(source, ['compare_time'], false)),
        false
      ),
      avoidRepeats: boolValue(
        firstValue(repeats, ['avoidRepeats'], firstValue(source, ['avoid_repeats'], false)),
        false
      ),
      allowedRepeats: numberValue(
        firstValue(repeats, ['allowedRepeats'], firstValue(source, ['allowed_repeats'], 0)),
        0
      ),
      repeatsWithinDays: numberValue(
        firstValue(repeats, ['repeatsWithinDays'], firstValue(source, ['repeats_within_days'], 0)),
        0
      ),
      useSeriesRecording: boolValue(
        firstValue(series, ['useSeriesRecording'], firstValue(source, ['use_series_recording'], false)),
        false
      ),
      keepRecordings: numberValue(
        firstValue(series, ['keepRecordings'], firstValue(source, ['keep_recs'], 0)),
        0
      ),
      deleteMode: numberValue(firstValue(series, ['deleteMode'], firstValue(source, ['del_mode'], 0)), 0),
      searchTimerAction: numberValue(
        firstValue(series, ['searchTimerAction'], firstValue(source, ['search_timer_action'], 0)),
        0
      ),
      blacklistMode: numberValue(
        firstValue(blacklist, ['blacklistMode'], firstValue(source, ['blacklist_mode'], 0)),
        0
      ),
      blacklistIds: String(
        firstValue(blacklist, ['blacklistIds'], firstValue(source, ['blacklist_ids'], ''))
      ),
      mode: numberValue(firstValue(match, ['mode'], firstValue(source, ['mode'], 0)), 0),
      matchCase: boolValue(
        firstValue(match, ['matchCase'], firstValue(source, ['match_case'], false)),
        false
      ),
      tolerance: numberValue(
        firstValue(match, ['tolerance'], firstValue(source, ['tolerance'], 0)),
        0
      ),
      summaryMatch: numberValue(
        firstValue(match, ['summaryMatch'], firstValue(source, ['summary_match'], 0)),
        0
      ),
      useExtendedEpgInfo: boolValue(
        firstValue(extended, ['useExtendedEpgInfo'], firstValue(source, ['use_ext_epg_info'], false)),
        false
      ),
      extendedEpgInfo: String(
        firstValue(extended, ['extendedEpgInfo'], firstValue(source, ['ext_epg_info'], ''))
      ),
      ignoreMissingEpgCategories: boolValue(
        firstValue(
          extended,
          ['ignoreMissingEpgCategories'],
          firstValue(source, ['ignore_missing_epg_cats'], false)
        ),
        false
      ),
      contentDescriptors: String(
        firstValue(extended, ['contentDescriptors'], firstValue(source, ['content_descriptors'], ''))
      ),
      useInFavorites: boolValue(
        firstValue(validity, ['useInFavorites'], firstValue(source, ['use_in_favorites'], false)),
        false
      ),
      activeFrom: String(
        firstValue(validity, ['activeFrom'], firstValue(source, ['use_as_searchtimer_from'], ''))
      ),
      activeUntil: String(
        firstValue(validity, ['activeUntil'], firstValue(source, ['use_as_searchtimer_til'], ''))
      ),
      pauseOnRecordings: boolValue(
        firstValue(actions, ['pauseOnRecordings'], firstValue(source, ['pause_on_recs'], false)),
        false
      ),
      switchMinutesBefore: numberValue(
        firstValue(actions, ['switchMinutesBefore'], firstValue(source, ['switch_min_before'], 0)),
        0
      ),
      unmuteSoundOnSwitch: boolValue(
        firstValue(actions, ['unmuteSoundOnSwitch'], firstValue(source, ['unmute_sound_on_switch'], false)),
        false
      ),
      deleteRecordingsAfterDays: numberValue(
        firstValue(actions, ['deleteRecordingsAfterDays'], firstValue(source, ['del_recs_after_days'], 0)),
        0
      ),
      deleteAfterCountRecordings: numberValue(
        firstValue(
          actions,
          ['deleteAfterCountRecordings'],
          firstValue(source, ['del_after_count_recs'], 0)
        ),
        0
      ),
      deleteAfterDaysOfFirstRecording: numberValue(
        firstValue(
          actions,
          ['deleteAfterDaysOfFirstRecording'],
          firstValue(source, ['del_after_days_of_first_rec'], 0)
        ),
        0
      ),
      updateSafe: false
  ...
  function installStyles() {
    if (typeof document === 'undefined' ||
        document.getElementById('vdr-suite-searchtimer-workflow-styles')) {
      return;
    }
...
@media(max-width:760px){.searchtimer-card-summary{grid-template-columns:minmax(0,1fr)}.searchtimer-badges{justify-content:flex-start}.searchtimer-form-grid{grid-template-columns:minmax(0,1fr)}.searchtimer-field.wide{grid-column:auto}.searchtimer-actions{display:grid;grid-template-columns:minmax(0,1fr)}.searchtimer-actions button{width:100%}.searchtimer-meta-grid{grid-template-columns:minmax(0,1fr)}}`;
  ...
  function buildCreatePayload(form, template) {
    ...
    return {
      ...
      marginStartMinutes: numberValue(
        formValue(form, 'marginStartMinutes', source.marginStartMinutes),
        0
      ),
      marginStopMinutes: numberValue(
        formValue(form, 'marginStopMinutes', source.marginStopMinutes),
        0
      ),
      useVps: boolValue(formValue(form, 'useVps', source.useVps), false),
      useChannel: String(channels || '').trim() !== '' ? 1 : 0,
      channels,
      ...
      durationMinMinutes: numberValue(
        formValue(form, 'durationMinMinutes', source.durationMinMinutes),
        0
      ),
      durationMaxMinutes: numberValue(
        formValue(form, 'durationMaxMinutes', source.durationMaxMinutes),
        0
      ),
      useDayOfWeek: false,
      dayOfWeek: 0,
      ...
      repeatsWithinDays: numberValue(
        formValue(form, 'repeatsWithinDays', source.repeatsWithinDays),
        0
      ),
      compareTitle: boolValue(formValue(form, 'compareTitle', source.compareTitle), true),
      compareSubtitle: boolValue(
        formValue(form, 'compareSubtitle', source.compareSubtitle),
        false
      ),
  ...
  function validateCreatePayload(payload) {
    ...
    if (String(payload.query || '').trim() === '') {
      errors.push('Suchbegriff fehlt.');
    }
    if (String(payload.name || '').trim() === '') {
      payload.name = String(payload.query || '').trim();
    }
    ...
    if (payload.useDuration &&
        payload.durationMaxMinutes > 0 &&
        payload.durationMinMinutes > payload.durationMaxMinutes) {
  ...
  function renderPreview(target, data) {
    ...
    appendPreviewMetric(
      summary,
      'Treffer',
      String(firstValue(statistics, ['totalCount'], previewMatches(data).length))
    );
    ...
    if (matches.length === 0) {
      ...
      matches.slice(0, 40).forEach((match, index) => {
        const event = match && match.event && typeof match.event === 'object'
          ? match.event
          : match;
        ...
        card.appendChild(addText(
          ...
          String(firstValue(event, ['title', 'name'], 'Treffer ' + String(index + 1)))
        ));
  ...
  function populateCreateForm(timer) {
    ...
    Object.keys(values).forEach(name => {
      if (activeCreateForm.elements[name]) {
        activeCreateForm.elements[name].value = String(values[name] || '');
      }
    });
    ...
    [
      ...
      if (activeCreateForm.elements[name]) {
        activeCreateForm.elements[name].checked = Boolean(timer[name]);
      }
    });
  ...
  function createCard(timer) {
    ...
    appendMeta(
      meta,
      'Marge Start / Stop',
      String(timer.marginStartMinutes) + ' / ' + String(timer.marginStopMinutes) + ' Min.'
    );
    appendMeta(
      meta,
      'Suchfelder',
      [
        timer.compareTitle ? 'Titel' : '',
        timer.compareSubtitle ? 'Untertitel' : '',
        timer.compareSummary ? 'Beschreibung' : ''
      ].filter(Boolean).join(', ') || '-'
    );
    ...
    safety.textContent =
      'Vorhandene SearchTimer werden noch nicht direkt überschrieben: ' +
      'Der aktuelle Leservertrag liefert Zeitfenster und konkreten Wochentag nicht vollständig zurück. ' +
      '„Als Vorlage“ übernimmt die sicher bekannten Werte in einen neuen SearchTimer.';
    ...
    toggle.title =
      'Sicheres Ändern wird erst freigegeben, wenn der vollständige Leservertrag verfügbar ist.';
  ...
  function renderList(data) {
    ...
    summary.appendChild(addText(
      ...
      String(searchTimers.length) + ' SearchTimer geladen.'
    ));
    ...
    if (searchTimers.length === 0) {
      ...
      empty.appendChild(addText(
        document.createElement('p'),
        'Der Backend-Endpunkt hat keine SearchTimer geliefert.'
      ));
