...
(function (global) {
  ...
  function applyHomeBackdrop(url) {
    const detail = doc && typeof doc.getElementById === 'function'
      ? doc.getElementById('detail')
      : null;
    if (!detail || !detail.style) return false;
    const value = text(url);
    const cssValue = value
      ? 'url("' + value.replace(/"/g, '%22') + '")'
      : 'none';
    if (typeof detail.style.setProperty === 'function') {
      detail.style.setProperty('--media-home-backdrop-image', cssValue);
    } else {
      detail.style['--media-home-backdrop-image'] = cssValue;
    }
    return true;
  }
