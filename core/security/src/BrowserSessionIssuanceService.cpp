    ...
    const BrowserSessionIssuanceRequest& request)
{
    ...
    IssuedBrowserSession result;
    result.tokenId = material->tokenId;
    result.sessionId = material->sessionId;
    result.credentialId = material->credentialId;
    result.sessionCookieValue =
        material->tokenId + "." + material->sessionSecret;
    result.csrfToken = material->csrfSecret;
