...
{
    const auto& lp = left.localProviderSelection;
    const auto& rp = right.localProviderSelection;
    return left.commandId == right.commandId &&
        ...
        lp.backendId == rp.backendId &&
        lp.authorityDomain == rp.authorityDomain &&
        lp.providerId == rp.providerId &&
        lp.providerKind == rp.providerKind &&
        lp.ownershipGeneration == rp.ownershipGeneration &&
        lp.providerInstanceEpoch == rp.providerInstanceEpoch &&
        lp.providerGeneration == rp.providerGeneration &&
        lp.capabilityRevision == rp.capabilityRevision &&
        lp.requiredCapability == rp.requiredCapability;
}
