...
private:
    ...
    {
        while (value < input_.size() &&
               std::isspace(static_cast<unsigned char>(input_[value]))) ++value;
        return value;
    }
    bool parseValue(Value& value, int depth)
    {
        ...
        {
            value.kind = Kind::Boolean; value.boolValue = true; position_ += 4; return true;
        }
        if (input_.compare(position_, 5, "false") == 0)
        {
            value.kind = Kind::Boolean; value.boolValue = false; position_ += 5; return true;
        }
    ...
    bool parseObject(Value& value, int depth)
    {
        ...
        while (position_ < input_.size())
        {
            ...
            if (!value.objectValue.emplace(key, std::move(child)).second ||
...
std::string escape(const std::string& value)
{
    ...
    for(unsigned char character:value)
    {
        switch(character)
        {
            ...
            case '\t':out<<"\\t";break;default:if(character>=0x20U)out<<static_cast<char>(character);
        }
    }
    return out.str();
}
std::string stringArray(const std::vector<std::string>& values)
{
    ...
    for(std::size_t index=0;index<values.size();++index){if(index)out<<',';out<<'"'<<escape(values[index])<<'"';}
    out<<']';return out.str();
}
bool parseTypes(const Value& value,std::vector<std::string>& types)
{
    ...
    for(const Value& item:value.arrayValue)
    {
        if(item.kind!=Kind::String||
            (item.stringValue!="probe.noop"&&item.stringValue!="vdr.native.probe")||
...
bool readAssignment(const Value& value,BackendAgentCommandAssignment& a)
{
    if(value.kind!=Kind::Object||!exactKeys(value.objectValue,{
        "protocolVersion","requestId","correlationId","operationId","jobId","attemptId",
        "claimEpoch","commandId","backendId","agentId","agentInstanceId","backendGeneration",
        "commandType","payloadVersion","payload","requestFingerprint","verificationPolicy","assignedAt","deadline"}))return false;
    ...
    a.present=true;a.protocolVersion=protocol->stringValue;a.requestId=request->stringValue;
    a.correlationId=correlation->stringValue;a.operationId=operation->stringValue;a.jobId=job->stringValue;
    a.attemptId=attempt->stringValue;a.claimEpoch=claim->unsignedValue;a.commandId=command->stringValue;
    a.backendId=backend->stringValue;a.agentId=agent->stringValue;a.agentInstanceId=instance->stringValue;
    a.backendGeneration=generation->unsignedValue;a.commandType=type->stringValue;a.payloadVersion=payloadVersion->unsignedValue;
    a.payload=payload->stringValue;a.requestFingerprint=fingerprint->stringValue;a.verificationPolicy=verification->stringValue;
    a.assignedAt=static_cast<std::int64_t>(assigned->unsignedValue);a.deadline=static_cast<std::int64_t>(deadline->unsignedValue);
    return backendAgentCommandValidAssignment(a);
}

std::string assignmentJson(const BackendAgentCommandAssignment& a)
{
    std::ostringstream out;
    out<<"{\"protocolVersion\":\""<<escape(a.protocolVersion)<<"\",\"requestId\":\""<<escape(a.requestId)
       <<"\",\"correlationId\":\""<<escape(a.correlationId)<<"\",\"operationId\":\""<<escape(a.operationId)
       <<"\",\"jobId\":\""<<escape(a.jobId)<<"\",\"attemptId\":\""<<escape(a.attemptId)
       <<"\",\"claimEpoch\":"<<a.claimEpoch<<",\"commandId\":\""<<escape(a.commandId)
       <<"\",\"backendId\":\""<<escape(a.backendId)<<"\",\"agentId\":\""<<escape(a.agentId)
       <<"\",\"agentInstanceId\":\""<<escape(a.agentInstanceId)<<"\",\"backendGeneration\":"<<a.backendGeneration
       <<",\"commandType\":\""<<escape(a.commandType)<<"\",\"payloadVersion\":"<<a.payloadVersion
       <<",\"payload\":\""<<escape(a.payload)<<"\",\"requestFingerprint\":\""<<escape(a.requestFingerprint)
       <<"\",\"verificationPolicy\":\""<<escape(a.verificationPolicy)<<"\",\"assignedAt\":"<<a.assignedAt
       <<",\"deadline\":"<<a.deadline<<'}';return out.str();
}
}
...
{
    std::ostringstream out;out<<"{\"protocolVersion\":\"vdr-suite-agent/1\",\"backendId\":\""<<escape(request.backendId)
       <<"\",\"agentInstanceId\":\""<<escape(request.agentInstanceId)<<"\",\"backendGeneration\":"<<request.backendGeneration
       <<",\"supportedCommandTypes\":"<<stringArray(request.supportedCommandTypes)<<'}';return out.str();
}
...
{
    Value root;if(!Parser(body).parse(root)||root.kind!=Kind::Object){reason="invalid_command_poll_response";return false;}
    const Value* has=get(root.objectValue,"hasAssignment",Kind::Boolean);const Value* code=get(root.objectValue,"reasonCode",Kind::String);
...
{
    std::ostringstream out;out<<"{\"hasAssignment\":"<<(result.assignment.present?"true":"false")
       <<",\"reasonCode\":\""<<escape(result.reasonCode)<<'"';
    if(result.assignment.present) out<<",\"assignment\":"<<assignmentJson(result.assignment);
    out<<'}';
    return out.str();
}
bool parseBackendAgentCommandReceiptJson(const std::string& body,BackendAgentCommandReceipt& r,std::string& reason)
{
    Value root;if(!Parser(body).parse(root)||root.kind!=Kind::Object||!exactKeys(root.objectValue,
    ...
    const auto s=[&](const char* key){return get(root.objectValue,key,Kind::String);};const auto u=[&](const char* key){return get(root.objectValue,key,Kind::Unsigned);};
    const Value *protocol=s("protocolVersion"),*command=s("commandId"),*finger=s("requestFingerprint"),*job=s("jobId"),*attempt=s("attemptId"),*claim=u("claimEpoch"),*backend=s("backendId"),*agent=s("agentId"),*instance=s("agentInstanceId"),*generation=u("backendGeneration"),*category=s("receiptCategory"),*received=u("receivedAt"),*code=s("reasonCode");
    if(!protocol||!command||!finger||!job||!attempt||!claim||!backend||!agent||!instance||!generation||!category||!received||!code){reason="invalid_command_receipt_payload";return false;}
    r.protocolVersion=protocol->stringValue;r.commandId=command->stringValue;r.requestFingerprint=finger->stringValue;r.jobId=job->stringValue;r.attemptId=attempt->stringValue;r.claimEpoch=claim->unsignedValue;r.backendId=backend->stringValue;r.agentId=agent->stringValue;r.agentInstanceId=instance->stringValue;r.backendGeneration=generation->unsignedValue;r.receiptCategory=category->stringValue;r.receivedAt=static_cast<std::int64_t>(received->unsignedValue);r.reasonCode=code->stringValue;
    if(!backendAgentCommandValidReceipt(r)){reason="invalid_command_receipt_payload";return false;}reason="command_receipt_parsed";return true;
}
std::string serializeBackendAgentCommandReceiptJson(const BackendAgentCommandReceipt& r)
{
    std::ostringstream out;out<<"{\"protocolVersion\":\"vdr-suite-agent/1\",\"commandId\":\""<<escape(r.commandId)<<"\",\"requestFingerprint\":\""<<escape(r.requestFingerprint)<<"\",\"jobId\":\""<<escape(r.jobId)<<"\",\"attemptId\":\""<<escape(r.attemptId)<<"\",\"claimEpoch\":"<<r.claimEpoch<<",\"backendId\":\""<<escape(r.backendId)<<"\",\"agentId\":\""<<escape(r.agentId)<<"\",\"agentInstanceId\":\""<<escape(r.agentInstanceId)<<"\",\"backendGeneration\":"<<r.backendGeneration<<",\"receiptCategory\":\""<<escape(r.receiptCategory)<<"\",\"receivedAt\":"<<r.receivedAt<<",\"reasonCode\":\""<<escape(r.reasonCode)<<"\"}";return out.str();
}
bool parseBackendAgentCommandResultJson(const std::string& body,BackendAgentCommandResult& r,std::string& reason)
{
    Value root;if(!Parser(body).parse(root)||root.kind!=Kind::Object||!exactKeys(root.objectValue,
    ...
    const auto s=[&](const char* key){return get(root.objectValue,key,Kind::String);};const auto u=[&](const char* key){return get(root.objectValue,key,Kind::Unsigned);};
    const Value *protocol=s("protocolVersion"),*command=s("commandId"),*finger=s("requestFingerprint"),*job=s("jobId"),*attempt=s("attemptId"),*claim=u("claimEpoch"),*backend=s("backendId"),*agent=s("agentId"),*instance=s("agentInstanceId"),*generation=u("backendGeneration"),*dispatch=s("dispatchState"),*verification=s("verificationState"),*category=s("resultCategory"),*error=s("errorCategory"),*retry=s("retryClassification"),*diagnostics=s("boundedDiagnostics"),*completed=u("completedAt");
    if(!protocol||!command||!finger||!job||!attempt||!claim||!backend||!agent||!instance||!generation||!dispatch||!verification||!category||!error||!retry||!diagnostics||!completed){reason="invalid_command_result_payload";return false;}
    r.protocolVersion=protocol->stringValue;r.commandId=command->stringValue;r.requestFingerprint=finger->stringValue;r.jobId=job->stringValue;r.attemptId=attempt->stringValue;r.claimEpoch=claim->unsignedValue;r.backendId=backend->stringValue;r.agentId=agent->stringValue;r.agentInstanceId=instance->stringValue;r.backendGeneration=generation->unsignedValue;r.dispatchState=dispatch->stringValue;r.verificationState=verification->stringValue;r.resultCategory=category->stringValue;r.errorCategory=error->stringValue;r.retryClassification=retry->stringValue;r.boundedDiagnostics=diagnostics->stringValue;r.completedAt=static_cast<std::int64_t>(completed->unsignedValue);
    if(!backendAgentCommandValidResult(r)){reason="invalid_command_result_payload";return false;}reason="command_result_parsed";return true;
}
std::string serializeBackendAgentCommandResultJson(const BackendAgentCommandResult& r)
{
    std::ostringstream out;out<<"{\"protocolVersion\":\"vdr-suite-agent/1\",\"commandId\":\""<<escape(r.commandId)<<"\",\"requestFingerprint\":\""<<escape(r.requestFingerprint)<<"\",\"jobId\":\""<<escape(r.jobId)<<"\",\"attemptId\":\""<<escape(r.attemptId)<<"\",\"claimEpoch\":"<<r.claimEpoch<<",\"backendId\":\""<<escape(r.backendId)<<"\",\"agentId\":\""<<escape(r.agentId)<<"\",\"agentInstanceId\":\""<<escape(r.agentInstanceId)<<"\",\"backendGeneration\":"<<r.backendGeneration<<",\"dispatchState\":\""<<escape(r.dispatchState)<<"\",\"verificationState\":\""<<escape(r.verificationState)<<"\",\"resultCategory\":\""<<escape(r.resultCategory)<<"\",\"errorCategory\":\""<<escape(r.errorCategory)<<"\",\"retryClassification\":\""<<escape(r.retryClassification)<<"\",\"boundedDiagnostics\":\""<<escape(r.boundedDiagnostics)<<"\",\"completedAt\":"<<r.completedAt<<'}';return out.str();
}
