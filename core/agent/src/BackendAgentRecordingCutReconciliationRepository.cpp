...
bool expectedEditedRecordingKey(
    ...
    std::string& editedRecordingKey)
{
    ...
    editedRecordingKey = diagnostics.substr(prefix.size(), 32U);
    return vdrsuite::agent::backendAgentRecordingCutRevisionTokenValid(
        editedRecordingKey);
}
    ...
    BackendAgentRecordingCutVerification& verification)
{
    ...
    verification.editedRecordingKey = text(statement, 6);
    ...
    return backendAgentCommandSafeIdentifier(verification.commandId) &&
        ...
        vdrsuite::agent::backendAgentRecordingCutRevisionTokenValid(
            verification.recordingKey) &&
        vdrsuite::agent::backendAgentRecordingCutRevisionTokenValid(
            verification.expectedMarksRevision) &&
        vdrsuite::agent::backendAgentRecordingCutRevisionTokenValid(
            verification.editedRecordingKey) &&
        verification.editedRecordingKey != verification.recordingKey &&
        verification.verifiedAt > 0;
}

bool exactVerificationRequest(
    const BackendAgentRecordingCutVerification& verification,
    const std::string& commandId,
    ...
    const std::string& editedRecordingKey)
{
    return verification.present &&
        ...
        verification.editedRecordingKey == editedRecordingKey;
}
}

bool BackendAgentCommandRepository::ensureRecordingCutReconciliationSchema()
{
    return database_.execute(
        "CREATE TABLE IF NOT EXISTS backend_agent_recording_cut_readbacks ("
        ...
        "edited_recording_key TEXT NOT NULL,verified_at INTEGER NOT NULL,"
        "FOREIGN KEY(command_id) REFERENCES backend_agent_commands(command_id));") &&
        database_.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS "
            "idx_backend_agent_recording_cut_readback_operation "
            "ON backend_agent_recording_cut_readbacks(backend_id,operation_id);");
}

std::vector<BackendAgentRecordingCutReconciliationCandidate>
BackendAgentCommandRepository::recordingCutReconciliationCandidates() const
{
    using namespace vdrsuite::agent;
    std::vector<BackendAgentRecordingCutReconciliationCandidate> candidates;
    ...
    const std::string sql = std::string("SELECT ") + AssignmentColumns +
        ...
        "WHERE c.command_type='vdr.recording.cut' "
    ...
    {
        BackendAgentRecordingCutReconciliationCandidate candidate;
        ...
        if (!backendAgentCommandValidAssignment(candidate.assignment) ||
            candidate.assignment.commandType != kBackendAgentRecordingCutCommandType ||
            candidate.assignment.verificationPolicy != "readback_required" ||
            !backendAgentRecordingCutParsePayload(
            ...
            !expectedEditedRecordingKey(
                diagnostics, candidate.assignment.commandId, editedRecordingKey) ||
            editedRecordingKey == payload.recordingKey)
        {
            continue;
        }
        ...
        candidate.editedRecordingKey = editedRecordingKey;
...
BackendAgentRecordingCutVerification
BackendAgentCommandRepository::recordingCutVerificationForOperation(
    ...
    const std::string& operationId) const
{
    ...
    const char* sql =
        ...
        "r.recording_key,r.expected_marks_revision,r.edited_recording_key,"
        "r.verified_at FROM backend_agent_recording_cut_readbacks r "
        "JOIN backend_agent_commands c ON c.command_id=r.command_id "
        "WHERE r.backend_id=? AND r.operation_id=? "
        "AND c.command_type='vdr.recording.cut' AND c.state='completed' "
        "LIMIT 1;";
    if (sqlite3_prepare_v2(
...
bool BackendAgentCommandRepository::verifyRecordingCutResult(
    const std::string& commandId,
    const std::string& requestFingerprint,
    const std::string& recordingKey,
    const std::string& expectedMarksRevision,
    ...
    std::string& reasonCode)
{
    ...
    if (!backendAgentCommandSafeIdentifier(commandId) ||
        ...
        !backendAgentRecordingCutRevisionTokenValid(recordingKey) ||
        !backendAgentRecordingCutRevisionTokenValid(expectedMarksRevision) ||
        !backendAgentRecordingCutRevisionTokenValid(editedRecordingKey) ||
        editedRecordingKey == recordingKey || observedAt <= 0)
    {
        reasonCode = "recording_cut_readback_invalid";
        return false;
    }
    ...
    const char* existingSql =
        ...
        "recording_key,expected_marks_revision,edited_recording_key,verified_at "
        "FROM backend_agent_recording_cut_readbacks WHERE command_id=?;";
    if (sqlite3_prepare_v2(
        ...
        !bindText(existingStatement, 1, commandId))
    {
        ...
        reasonCode = "recording_cut_readback_repository_error";
        return false;
    }
    if (sqlite3_step(existingStatement) == SQLITE_ROW)
    {
        ...
        if (valid && exactVerificationRequest(
                ...
                editedRecordingKey))
        {
            reasonCode = "recording_cut_readback_replayed";
            return true;
        }
        verification = {};
        reasonCode = "recording_cut_readback_conflict";
        return false;
    }
    sqlite3_finalize(existingStatement);
    ...
    {
        sqlite3_finalize(statement);
        reasonCode = "recording_cut_readback_candidate_not_found";
        return false;
    }
    ...
    if (!backendAgentCommandValidAssignment(assignment) ||
        ...
        !expectedEditedRecordingKey(
            diagnostics, assignment.commandId, expectedEditedKey))
    {
        reasonCode = "recording_cut_readback_candidate_conflict";
        return false;
    }
    if (expectedEditedKey == recordingKey ||
        editedRecordingKey != expectedEditedKey)
    {
        reasonCode = "recording_cut_readback_state_mismatch";
        return false;
    }
    ...
    if (selectedProvider->backendId != assignment.backendId ||
        ...
        selectedProvider->requiredCapability != kBackendAgentRecordingCutCapability)
    {
        reasonCode = "recording_cut_provider_selection_mismatch";
        return false;
    }
    ...
    const char* insertSql =
        "INSERT INTO backend_agent_recording_cut_readbacks("
        "command_id,request_fingerprint,operation_id,backend_id,recording_key,"
        "expected_marks_revision,edited_recording_key,verified_at) "
        "VALUES(?,?,?,?,?,?,?,?);";
    bool ok = sqlite3_prepare_v2(
        ...
        bindText(insert, 7, editedRecordingKey) &&
    ...
    reasonCode = "recording_cut_readback_verified";
    return true;
}
