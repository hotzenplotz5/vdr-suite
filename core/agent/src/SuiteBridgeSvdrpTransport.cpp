...
public:
    ...
    explicit ScopedFd(const int fd)
        : fd_(fd)
    {
    }
...
const char* commandText(const SuiteBridgeLocalCommand command)
{
    switch (command)
    {
...
TimePoint boundedDeadline(
    ...
    const std::chrono::milliseconds phaseTimeout)
{
    return std::min(operationDeadline, Clock::now() + phaseTimeout);
}

int pollTimeoutMilliseconds(const TimePoint deadline)
{
    ...
    const auto roundedMilliseconds =
        (remainingMicroseconds + 999) / 1000;

    if (roundedMilliseconds > INT_MAX)
    {
        return INT_MAX;
    }

    return static_cast<int>(std::max<std::int64_t>(1, roundedMilliseconds));
}

IoStatus waitForSocket(
    const int fd,
    const short events,
...
bool configureSocket(const int fd)
{
    ...
    std::string& line)
{
    while (true)
    {
        ...
        char buffer[512];
        const std::size_t remaining = maximumBytes - receivedBytes;
        const std::size_t requested = std::min(remaining, sizeof(buffer));
...
SuiteBridgeCommandReply SuiteBridgeSvdrpTransport::execute(
    const SuiteBridgeLocalCommand command)
{
