...
class NativeProbeTimeoutTransport final:public IBackendAgentNativeProbeTransport {
public:
 SuiteBridgeCommandReply discoverNativeProbe()override{return{SuiteBridgeTransportStatus::Timeout,0,{},"timeout"};}
 SuiteBridgeCommandReply executeNativeProbe(const SuiteBridgeNativeProbeRequest&)override{return{SuiteBridgeTransportStatus::Timeout,0,{},"timeout"};}
 SuiteBridgeCommandReply readNativeProbe(const SuiteBridgeNativeProbeReadbackRequest&)override{return{SuiteBridgeTransportStatus::Timeout,0,{},"timeout"};}
};
...
int main(){
 ...
 NativeProbeTimeoutTransport timeoutTransport;SuiteBridgePrioritizedNativeProbeTransport noReplay(timeoutTransport,nativeCompatibility);auto timeoutReply=noReplay.executeNativeProbe(probe);assert(timeoutReply.transportStatus==SuiteBridgeTransportStatus::Timeout);assert(nativeCompatibility.executeCalls.load()==1);
