...
{
public:
    explicit Server(std::string reply) : reply_(std::move(reply))
    {
...
int main()
{
    ...
    {
        Server server(
            "250 vdr-suite-rcut-state/1 " + key +
            " 1 ready 1 1 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa 1 4 2 0 0 " +
            edited + " 0 0\r\n");
        SuiteBridgeSvdrpTransport transport(configFor(server));
        const auto reply = transport.requestRecordingCutState(key);
        server.wait();
        assert(reply.transportSucceeded);
        assert(reply.replyCode == 250);
        assert(reply.payload.find("vdr-suite-rcut-state/1") == 0);
        assert(server.request() == "PLUG suitebridge RCUT " + key + "\r\n");
    }

    {
        SuiteBridgeSvdrpTransport transport;
        assert(!transport.requestRecordingCutState(
            "C94D0EB9958A85079F81F059A436003C").transportSucceeded);
        assert(!transport.requestRecordingCutState(
            "/srv/vdr/video/test.rec").transportSucceeded);
        assert(!transport.requestRecordingCutState("%2fsrv%2fvdr").transportSucceeded);
    }

    {
        Server server("451 Recording cut state unavailable\r\n");
        SuiteBridgeSvdrpTransport transport(configFor(server));
        const auto reply = transport.requestRecordingCutState(key);
        server.wait();
        assert(!reply.transportSucceeded);
        assert(reply.replyCode == 451);
        assert(reply.payload == "Recording cut state unavailable");
    }
