...
int main()
{
    ...
    SequencedHttpClient httpClient({
        ...
        jsonResponse("{\"dirs\":[\"Doku\",\"Serien~Krimi\"],\"count\":2,\"total\":2}"),
        jsonResponse("{\"blacklists\":[{\"id\":3,\"search\":\"Sport\"},{\"id\":7,\"search\":\"Wiederholung\"}],\"count\":2,\"total\":2}"),
        jsonResponse("{\"ext_epg_info\":[{\"id\":1,\"name\":\"category\",\"values\":[\"movie\",\"series\"],\"config\":\"category|movie,series\"}],\"count\":1,\"total\":1}")
    });
    ...
    assert(realCatalog.recordingDirectories().at(1).path() == "Serien~Krimi");
    ...
    assert(realCatalog.blacklists().at(1).search() == "Wiederholung");
    ...
    assert(realCatalog.extendedEpgInfos().at(0).values().at(1) == "series");
    assert(realCatalog.extendedEpgInfos().at(0).config() == "category|movie,series");
